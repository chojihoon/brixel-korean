...
class OLED_uno_lib {  
  ...
  void init() {    
    ...
    for (uint8_t i = 0; i < sizeof ( ssd1306_init_sequence ); i++) command_write( pgm_read_byte( &ssd1306_init_sequence[i] ) );
  }  
  void clearDisplay() {
    int16_t i, j;
  ...
  void gotoXY( int16_t x, int16_t y ) {  
    ...
    #ifdef OLED_13inch
       command_write( 0x02 + ( FONT_SIZE * x & 0x0f ) );        //set low col address 1.3"
    #else
       command_write( 0x00 + ( FONT_SIZE * x & 0x0f ) );        //set low col address 0.96"
    #endif
    command_write( 0x10 + ( ( FONT_SIZE * x >> 4 ) & 0x0f ) );//set high col address
  }
  void drawBar( int16_t x, int16_t y, int16_t value ){
    int i;
    command_write( 0xb0 + y + i );                           //set page address
    command_write( 0x00 + ( FONT_SIZE * x & 0x0f ) );        //set low col address
    command_write( 0x10 + ( ( FONT_SIZE * x >> 4 ) & 0x0f) );
    for(i=0;i<128;i++) {
